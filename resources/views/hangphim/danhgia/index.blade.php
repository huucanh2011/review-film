  ...
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-hover" id="dataTable" width="100%">
        ...
        <tbody>
          @foreach(get_danhGiaHp() as $danhGia)
          <tr>
            <td><i class="fas fa-star text-warning"></i> {{ $danhGia->diem }}</td>
            <td>{{ $danhGia->noi_dung }}</td>
            <td>{{ $danhGia->phim()->first()->ten_chinh }}</td>
            <td>           
              <button type="button" class="btn btn-success btn-circle btn-sm js-btn-xem-ngdanhgia"
                id-ngdanhgia="{{ $danhGia->user()->first()->id }}" >
                <span class="far fa-eye"></span>
              </button>
              {{ $danhGia->user()->first()->ten_hien_thi }}
            </td>
            <td>{{ $danhGia->created_at->diffForHumans(get_now()) }}</td>
            <td class="text-center">
            <button class="btn btn-danger btn-sm btn-circle js-btn-xoa-danh-gia-hp" data-id="{{ $danhGia->id }}"
...
<div class="modal fade" id="modal-show-tt-ngdanhgia" tabindex="-1" role="dialog" aria-labelledby="showngdanhgia" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <div class="col-4">
            <img id="show-hinh" src="" class="rounded-circle" width="150px">            
          </div>
          ...
          <div class="col-8">
            <div class="form-group row">
              ...
              <div class="col-7">
                <input type="text" readonly class="form-control-plaintext" id="show-email">
              </div>
            </div>
            <div class="form-group row">
              <label class="col-5 col-form-label" for="show-ten">Tên hiển thị:</label>
              <div class="col-7">
                <input type="text" readonly class="form-control-plaintext" id="show-ten">
              </div>
            </div>
          </div>
